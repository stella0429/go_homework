...
5.结合课上学习内容，对现有PHP开发的视频相关项目优化构想：
1）用户首次观看视频活动奖励等场景，存储用户登录记录，可以改成用redis的bitmap（布隆过滤器），并且可以做本地缓存，通过head头传递登录情况（最新登录时间字段记录）；
2）用户实时观看视频记录上报，可以做如下优化：
a.加一层结构，利用cdn上报到统一入口新加的这层结构做聚合汇总，批量上报到后端服务器做处理，减少后台服务端压力。
b.观看记录做redis缓存同时，加一层kafka消息队列的投递，做聚合，消峰等作用，再批量上报到大数据那边。
c.预防redis穿透，导致数据库压力突然暴增以及redis的读写压力等，构建缓存时，可以进行优化，比如利用go的sync.Once，防止并发重复构建，并且设置一个类似全局
变量开关，一小段时间内有效，减少重复构建。
3）前端做限流控制优化。
4）后端通过滑动窗口算法实时监测流量，自动限流，扩缩容等
5）购买课程时，订单的唯一性，可以优化为雪花算法。
6）and so on...
